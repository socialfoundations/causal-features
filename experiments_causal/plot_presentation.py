...
fig.savefig(
    str(Path(__file__).parents[0] / f"plots_presentation/plot_introduction.pdf"),
    bbox_inches="tight",
)
...
for index, experiment_name in enumerate(experiments):
    ...
    fig.savefig(
    str(Path(__file__).parents[0] / f"plots_presentation/plot_{experiment_name}_anticausal.pdf"),
    bbox_inches="tight",
    )
...
for index, experiment_name in enumerate(experiments):
    ...
    fig.savefig(
    str(Path(__file__).parents[0] / f"plots_presentation/plot_{experiment_name}.pdf"),
    bbox_inches="tight",
    )
...
for index, experiment_name in enumerate(experiments):
    ...
    fig.savefig(
    str(Path(__file__).parents[0] / f"plots_presentation/plot_{experiment_name}_causalml.pdf"),
    bbox_inches="tight",
    )
...
for index, experiment_name in enumerate(experiments):
    ...
    #############################################################################
    # plot shift gap for all features
    #############################################################################
    eval_plot = eval_all[eval_all["features"] == "all"]
    eval_plot = eval_plot[
        (eval_plot["model"] != "irm")
    ...
    points.sort_values("id_test", inplace=True)
    ax.plot(
        [points["ood_test"][0],points["id_test"][0]],
        [points["ood_test"][0],points["ood_test"][0]],
        color="tab:blue",
        linestyle="-",
    )
    ...
    xmin, xmax = ax.set_xlim()
    ymin, ymax = ax.set_ylim()
    start_lim = max(xmin-xmin*0.01, ymin)
    end_lim = min(xmax, ymax)
    ax.plot([start_lim, end_lim], [start_lim, end_lim], color="k")

    ax.set_xlim(start_lim,xmax)
    ax.set_ylim(start_lim,ymax)

    fig.savefig(
    str(Path(__file__).parents[0] / f"plots_presentation/plot_lack_of_domaingeneralization.pdf"),
    bbox_inches="tight",
    )

# %%
    
wilds = pd.read_csv("/Users/vnastl/Seafile/My Library/mpi project causal vs noncausal/accuracy-on-the-line/results.csv")

wilds = wilds[(wilds["train_set"] == 'FMoW-train')&(wilds["shift_set"] == 'FMoW-ood_test')]
eval_plot = pd.DataFrame()
eval_plot["id_test"] = wilds["test_accuracy"]
eval_plot["ood_test"] = wilds["shift_accuracy"]
def string_to_tuple(s):
    return eval(s)
eval_plot["id_test_ub"] =  wilds["test_accuracy_ci"].apply(string_to_tuple).apply(lambda x:x[1])
eval_plot["ood_test_ub"] =  wilds["shift_accuracy_ci"].apply(string_to_tuple).apply(lambda x:x[1])


fig, ax = plt.subplots() 

ax.set_title("fMoW-WILDS")
...
mask = paretoset(points, sense=["max", "max"])
points = points[mask]
points = points[points["id_test"] >= 0.5]
# get extra points for the plot
new_row = pd.DataFrame(
    {
        "id_test": [xmin, max(points["id_test"])],
...
xmin, xmax = ax.set_xlim()
ymin, ymax = ax.set_ylim()
xmin, xmax = xmin-0.01*xmin, xmax+0.01*xmax
ymin, ymax = ymin-0.01*ymin, ymax+0.01*ymax
...
fig.savefig(
    str(Path(__file__).parents[0] / f"plots_presentation/plot_lack_of_domaingeneralization_wilds.pdf"),
    bbox_inches="tight",
    )