...
if __name__ == "__main__":
    ...
    def launch_experiments_jobs(
        ...
        arguments = {
            "pc": f'--save /home/vnastl/causal-features/experiments_causal/causal_discovery/pc.R',
            "fci": f'--save /home/vnastl/causal-features/experiments_causal/causal_discovery/fci.R',
        }

        # Construct job description
        job_description = htcondor.Submit(
            {
                "executable": "/usr/lib/R/bin/Rscript",
                # "arguments": "foo.py",    # NOTE: used for testing
                "arguments": (
                    f"{arguments(exp_obj.model)} " 
                    f"{exp_obj.name} "
                    "0.01 "
                ...
                "periodic_remove": f"(JobStatus == 2) && (time() - EnteredCurrentStatus) > (48 * 3600)",
            }
        )
