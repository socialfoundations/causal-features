...
## estimate CPDAG
fci_results <- fci(suffStat,
                 ## independence test: G^2 statistic
                 indepTest = disCItest, alpha = alpha, labels = V, verbose = TRUE)

save(fci_results,file=paste(paste(task,"fci","alpha",format(alpha, scientific = FALSE),sep="_"),"RData",sep="."))

# if (require(Rgraphviz)) {
#   ## show estimated CPDAG
#   png(file=paste(paste(task,"fci","alpha",format(alpha, scientific = FALSE),sep="_"),"png",sep="."))
#   plot(fci_results, main = "Estimated CPDAG")
#   dev.off()
# }